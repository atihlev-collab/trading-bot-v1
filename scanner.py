...
def analyze_symbol(symbol):
    try:
        ...
        ts = _num(trend_strength(low_fast, low_slow).iloc[i])
        adx_now = _num(adx_v.iloc[i]) or 0.0
        hist_now = _num(macd_hist.iloc[i]) or 0.0
        volume_now = _num(low["volume"].iloc[i]) or 0.0
        volume_average = _num(vol_ma.iloc[i]) or 0.0
        ...
        if None in (
            high_close, high_trend_now, high_fast_now, high_slow_now,
            low_trend_now, low_fast_now, low_slow_now
        ):
            return None

        htf_bull = high_close > high_trend_now and high_fast_now > high_slow_now
        ltf_bull = price > low_trend_now and low_fast_now > low_slow_now

        open_price = _num(low["open"].iloc[i])
        if open_price is None or open_price <= 0:
            return None

        candle_body = abs(price - open_price) / open_price
        vol_ratio = _safe_ratio(volume_now, volume_average)

        # Score instead of rejecting every imperfect indicator.
        score = 0
        reasons = []
        ...
        if mom >= 0.006:
            score += 15
            reasons.append("MOM+")
        elif mom >= max(0.003, MIN_MOMENTUM):
            score += 10
            reasons.append("MOM")
        ...
        confidence_points = 0
        if htf_bull: confidence_points += 1
        if ltf_bull: confidence_points += 1
        if 52 <= r <= 68: confidence_points += 1
        if mom >= 0.003: confidence_points += 1
        if adx_now >= 25: confidence_points += 1
        if hist_now > 0: confidence_points += 1
        if vol_ratio >= 1.20: confidence_points += 1

        confidence = round((confidence_points / 7) * 100)
        buy_score = max(90, int(BUY_SCORE))

        if (
            score >= buy_score and confidence >= 85
            and htf_bull and ltf_bull
            and adx_now >= 20 and hist_now > 0
            and vol_ratio >= 1.15 and mom >= 0.003
            and r <= 68 and candle_body <= MAX_GREEN_CANDLE
        ):
            signal_type = "BUY"
        elif score >= 75:
            signal_type = "WATCH"
...
def scan_market():
    ...
    if signals:
        ...
        print(
            ...
            f"HTF={top['htf_bull']} "
            f"LTF={top['ltf_bull']} "
            f"CONF={top['confidence']} Q={top['quality']}"
        )
        if top.get("reasons"):
            print("[REASONS] " + ", ".join(top["reasons"]))
